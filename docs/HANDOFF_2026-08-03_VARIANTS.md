...
- 合规美术已由用户验收并切换为 `ready`；构建仍必须通过相同的 fail-closed 清单、哈希和泄漏检查，不能回退到展示版原图或占位图。
...
- 16 支国家队全部可选。
- 48 队完整赛程和对手数据。
- 16 支可选队的精确球衣，以及其余 32 支赛程球队使用的共享染色球衣方案。
- 正式比赛打平后的点球大战。
...
- 32 支仅参与赛程的球队入口和冗余精确球衣资源；48 队赛程不能因此被删除。
- 展示版的版权敏感原图。
...
- 6 项品牌美术（标题两帧、冠军图标、首页背景、聘书印章、更衣室背景）。
- 48 面国家队国旗。
- 16 个可选国家队队徽。
...
当前 `art-packs/compliant/manifest.json` 状态为 `ready`，69 项替换素材已经用户验收，聘书印章采用合规版排除策略。若未来任何受控槽位重新变为 `pending`，不得为了出包而：
...
- 合规素材当前为 `ready`，两个合规目标允许正式构建；一旦 manifest 回到 `pending`，涉及合规素材的构建必须安全失败。
...
### 2026-08-03 赛事 IP 合规改造进展（当前工作区，尚未提交）

- 两个合规版本已切换到独立的“国际足球冠军赛”品牌模块；展示版继续使用原品牌模块。
- 版权清单从 67 项扩展到 70 项，新增首页背景、聘书印章和更衣室背景；印章使用合规版排除策略并由代码绘制通用印章。
- 标题两帧、首页背景、更衣室、独立冠军杯图标和 16 枚国家队识别型二创徽章已经放入 `art-packs/compliant/`，尺寸与展示槽位一致且哈希不同。标题中央已改为原创金色足球冠军杯，发光帧仅在同一杯体后增加像素金色光环；记分牌和两个结算页也共用该杯体，不再使用星形勋章。
- 首页背景保留原羊皮纸旅行地图风格，只替换受限赛事文字和国旗图形；更衣室保留原米白构图，只替换赛事文字和杯形元素。
- 48 张队友提供的国家识别牌已从 `复古足球国家识别牌_48国_20260803.zip` 映射到 ASCII 旗帜路径，并按展示槽位精确适配尺寸。互动 Runtime 中 8 张内嵌旗帜已用相同批准源生成 `512×256` 预览并通过哈希与视觉复核；用户已确认完整 69 项素材联系表，`manifest.json` 已切换为 `ready`。
- 已增加合规产物内容/路径禁词扫描、展示原图哈希泄漏检查，以及互动 Runtime 内嵌旗帜从已批准旗帜源重写的门禁。
- 互动空间性能改造后已重新验证 40 个测试文件、433 项测试和 lint；三个版本的最终人工冒烟仍需完成。

### 2026-08-03 互动空间性能与画质改造（当前工作区，尚未提交）

- 修复开场缩远镜头：互动空间首帧在揭幕前强制回到球场中心并采用正式缩放，不再播放会露出球场外背景的全景缩放段。揭幕后使用 `camera.auto()` 退出自由镜头并恢复随球状态；不能只调用 `followBall()`，因为旧引擎在 `isFree=true` 时会将其作为空操作。
- 互动版重新启用首页空闲期的完整比赛 Runtime 预热，提前完成 PIXI、球场和首套球衣纹理装配；选队后只预热本场 Runtime 素材，不再并行预载无关的逐帧点球资源或整队头像。正式开赛若撞上尚未结束的预热，会等待预热收口后按用户所选球队快速重开，不会继承预热球队。
- 修复 `onMatchLoaded` 后 `_kickoffSnapped` 在互动训练局不及时到达时仍等待 900 帧（约 15 秒）的问题；互动版兜底改为 8 帧，揭幕函数自身继续负责球员开球位吸附，完整版保持原等待逻辑。正式包浏览器冷路径复测从约 16 秒降到 0.47 秒。
- 95% 之后改为“创建现场 / 装配球场与球衣 / 同步开球阵型”的连续阶段反馈，实际 Runtime 完成后才到 100%，不绕过真实就绪门禁。
- 互动空间可选球队由 4 支扩展到 16 支，16 套精确球衣和完整球员资料均保留；32 支仅赛程对手继续使用共享染色球衣。
- `match-quality` 压缩档已把球员、球衣和号码等比赛像素素材改为无损 PNG，主球场保持 96 色。用户确认平台上传门禁已经扩展到 15 MiB，版本配置、内部验证器和 `h5-validator` 都按 15 MiB 校验；实测把普通美术提高到 192 色会把 ZIP 增至约 11.24 MiB，但真机出现轻微卡顿且画质收益有限，因此正式档主动回到约 7.78 MiB，并未为了用满预算硬加资源。
- 互动版 WebGL 启用最近邻合成、关闭抗锯齿和整数像素对齐；为降低移动端填充率和发热，渲染预算由约 600 万物理像素、最高 2.0× 调整为约 400 万物理像素、最高 1.5×。球员模式默认缩放仍为 1.16，球员纹理继续无损，因此人物识别度不依赖高倍率抗锯齿。
- Runtime 人物纹理增加路径级缓存，逐帧应用时只更新实际变化的纹理、颜色和标签；互动版 5120×2560 球场底图装载后只补绘一次，不再在开场连续重绘大纹理。浏览器复核时 1280×720 视口对应 1920×1080 缓冲区，确认为 1.5×，随球镜头正常。
- 训练基地的门柱/横梁碰撞改为由 Runtime 精确碰撞信号在当帧直接触发 `postHit` 音效，正式比赛演出队列在训练模式下不再重复播放。
- 最新互动 ZIP 为 8,155,788 字节（约 7.78 MiB）、共 1,151 个文件，内部验证器及 `h5-validator` 均通过。

### 2026-08-03 三版本快速开赛逻辑（当前工作区，尚未提交）

- 首页空闲预热已经从互动版专用逻辑下沉为三版本公共逻辑：`showcase-full`、`compliant-full` 和 `compliant-interactive` 都会提前真实装配 PIXI、球场与首套球衣纹理，不再由两个完整版只预载 Runtime 脚本。
- 三版本进入比赛后统一只等待 8 帧取得可选的 `_kickoffSnapped` 标记；即使标记未到，`reveal()` 仍会把 22 人吸附到开球阵型，因此删除了完整版最长约 15 秒的空等路径。
- 两个完整版继续保留原有中心机位片头动画；片头结束时显式调用 `camera.auto()` 恢复随球镜头，避免快速揭幕后停在中圈。
- 浏览器以合规完整版现有球员存档复测：首页空闲预热后，从点击“继续征程”到正式比分界面约 291 ms。展示完整版和合规完整版均已成功构建为网页目录，未生成 ZIP。
...
> 请先完整阅读 `docs/HANDOFF_2026-08-03_VARIANTS.md`，再检查当前 Git 状态。以 `main` 为唯一业务源码，遵守三版本功能边界；只有 `compliant-interactive` 可以生成 ZIP。合规素材若为 pending 时不得绕过 fail-closed，也不要从旧包、旧分支或 stash 整体回滚。
