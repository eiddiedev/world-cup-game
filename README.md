...
- 当前自动门禁：25 个测试文件、336 项有效测试。已随退役 Phaser 表现层删除 17 项只验证旧代码的断言；自动通过仍不等于 53 场景逐项产品验收通过。
...
- `docs/balance/2026-07-20-algorithm-optimization-report.md`：当前平衡基线。
...
- `public/` 原始运行资源：`76,688,927 bytes`（73.14MiB）。
- `npm run build` 完整构建：`77,657,190 bytes`（74.06MiB）。
