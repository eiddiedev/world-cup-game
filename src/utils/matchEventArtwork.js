...
export function getMatchEventArtwork(event) {
  ...
  const varOutcome = event.type === 'var-result'
    ? event.detail?.outcome === 'disallowed' ? 'disallowed' : 'valid'
    : null
  const artworkKey = cardColor
    ? `card:${cardColor}`
    : varOutcome ? `var-result:${varOutcome}` : event.type
