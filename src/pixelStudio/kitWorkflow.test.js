...
describe('16-team kit production catalog', () => {
  ...
  it('locks exactly 16 teams and two kits per team without New Zealand', () => {
  ...
  it('passes every deterministic gold audit and records fixed PNG dimensions', () => {
    expect(audit.passedKitCount).toBe(32)
    expect(audit.rules.extractionMethod).toBe('direct-source-crop')
    expect(audit.rules.sourcePixelsRepainted).toBe(false)
    expect(audit.rules.runtimeStickerBorder).toBe(false)
    for (const entry of catalog.kits) {
      expect(entry.status).toBe('gold-pass')
      expect(entry.extractionMethod).toBe('direct-source-crop')
      const expectedSlots = entry.kitType === 'goalkeeper' ? KIT_SLOT_ORDER : KIT_SLOT_ORDER.slice(0, 8)
      expect(entry.files.map((file) => file.slotId).sort()).toEqual([...expectedSlots].sort())
      entry.files.forEach((file) => {
        expect(file.passed).toBe(true)
        expect(file.sha256).toHaveLength(64)
        expect(file.semiTransparentPixels).toBe(0)
        expect(file.lightBoundaryPixels).toBe(0)
